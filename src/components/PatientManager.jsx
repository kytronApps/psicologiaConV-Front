...
export default function PatientManager(props) {
  ...
  const [unlockedPatientIds, setUnlockedPatientIds] = useState([]);
  ...
  const filteredPatients = patientList.filter((pat) => {
    ...
    return code.includes(query) || pat.name.toLowerCase().includes(query) || pat.occupation.toLowerCase().includes(query);
  });
  ...
  const filteredFiles = currentPatientFiles.filter((file) => yearFilter === 'all' || file.year.toString() === yearFilter);
  ...
  const selectQuickMockFile = (name, content) => {
      ...
      <div id="pacientes_sidebar" className="lg:col-span-4 bg-white p-5 rounded-2xl border border-slate-100 shadow-sm">
        ...
        {showAddPatient && (
          <form onSubmit={handleCreatePatientSubmit} className="bg-slate-50 p-3.5 rounded-xl border border-slate-200 mb-3 space-y-2.5">
            ...
            <input id="new_pat_name" type="text" required placeholder="Nombre completo" value={newPatientName} onChange={(e) => setNewPatientName(e.target.value)} className="w-full px-2.5 py-1.5 text-xs border border-slate-200 rounded-lg bg-white text-slate-700 focus:outline-none focus:ring-1 focus:ring-emerald-500" />
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="block text-[9px] uppercase tracking-wider font-extrabold text-slate-400">Nacimiento</label>
                <input id="new_pat_birthdate" type="date" required value={newPatientBirthDate} onChange={(e) => setNewPatientBirthDate(e.target.value)} className="w-full mt-0.5 px-2.5 py-1.5 text-xs border border-slate-200 rounded-lg bg-white focus:outline-none focus:ring-1 focus:ring-emerald-500 text-slate-700" />
              </div>
              <div>
                <label className="block text-[9px] uppercase tracking-wider font-extrabold text-slate-400">Edad</label>
                <input id="new_pat_age" type="number" required placeholder="Edad" value={newPatientAge} onChange={(e) => setNewPatientAge(Number(e.target.value))} className="w-full mt-0.5 px-2.5 py-1.5 text-xs border border-slate-200 rounded-lg bg-white focus:outline-none focus:ring-1 focus:ring-emerald-500 text-slate-700" />
              </div>
            </div>
            <input type="text" placeholder="Ocupación / Oficio" value={newPatientOccupation} onChange={(e) => setNewPatientOccupation(e.target.value)} className="w-full px-2.5 py-1.5 text-xs border border-slate-200 rounded-lg bg-white focus:outline-none focus:ring-1 focus:ring-emerald-500 text-slate-700" />
            <input type="email" placeholder="Email" value={newPatientEmail} onChange={(e) => setNewPatientEmail(e.target.value)} className="w-full px-2.5 py-1.5 text-xs border border-slate-200 rounded-lg bg-white focus:outline-none" />
            <textarea placeholder="Sintomatología o notas introductorias..." rows={2} value={newPatientNotes} onChange={(e) => setNewPatientNotes(e.target.value)} className="w-full p-2 text-xs border border-slate-200 rounded-lg bg-white focus:outline-none" />
            <div className="flex justify-end gap-1.5 text-xs font-semibold pt-1">
              <button type="button" onClick={() => setShowAddPatient(false)} className="px-2.5 py-1 text-slate-500 hover:text-slate-700">Cancelar</button>
              <button type="submit" className="px-3 py-1 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg shadow-sm font-bold">Crear Registro</button>
            </div>
          </form>
        )}
          ...
          {filteredPatients.length === 0 ? (
            ...
            filteredPatients.map((patient) => {
              ...
              return (
                <div
                  key={patient.id}
                  onClick={() => { setSelectedPatientId(patient.id); setYearFilter('all'); setPasswordInput(''); setPasswordError(null); }}
                  id={`patient_item_${patient.id}`}
                  className={`flex items-center justify-between p-2.5 rounded-xl cursor-pointer transition-all border ${isSelected ? 'bg-emerald-50/75 border-emerald-200 text-slate-800 font-bold' : 'bg-white border-transparent hover:bg-slate-50/50 text-slate-600'}`}
                >
                  <div className="flex items-center gap-2.5">
                    {isUnlocked ? <FolderOpen className="h-4 w-4 text-emerald-600 flex-shrink-0" /> : <Folder className="h-4 w-4 text-slate-400 flex-shrink-0" />}
                    <span className="text-xs text-slate-800 font-mono tracking-tight font-bold">{getPatientFolderCode(patient)}</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <span className="text-[9px] font-bold px-1.5 py-0.1 bg-slate-100 rounded text-slate-500">{fileCount} f</span>
                    {isUnlocked ? <span className="text-[10px] text-emerald-600 font-bold">🔓</span> : <span className="text-[10px] text-slate-300">🔒</span>}
                  </div>
                </div>
              );
            })
          ...
          !unlockedPatientIds.includes(selectedPatientId) ? (
            <div id="patient_folder_locked_gate" className="bg-white p-10 rounded-3xl border border-slate-100 shadow-sm max-w-xl mx-auto text-center space-y-6 my-8">
              <div className="flex justify-center">
                <div className="bg-amber-50 text-amber-600 p-4 rounded-full border border-amber-100">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-slate-500 leading-relaxed max-w-md mx-auto">
                  Por estrictos motivos de confidencialidad, este expediente se encuentra encriptado localmente. Se requiere la contraseña de desbloqueo clínica.
                </p>
              </div>
              ...
              <form onSubmit={handleUnlockSubmit} className="space-y-4 max-w-xs mx-auto">
                ...
                />
                {passwordError && <p className="text-[11px] text-rose-600 font-bold">{passwordError}</p>}
                <button type="submit" className="w-full py-2.5 px-4 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs rounded-xl shadow transition-all cursor-pointer flex items-center justify-center gap-1.5">
                  🔓 Desbloquear Carpeta
                </button>
              </form>

              <div className="p-4 bg-indigo-50/70 border border-indigo-100 rounded-2xl text-left text-[11px] text-indigo-900 space-y-1.5">
                <p className="font-extrabold">💡 Indicaciones para la revisión:</p>
                <p className="text-[10px]">La contraseña clínica es el primer nombre en minúsculas + <code className="font-bold font-mono">123</code>.</p>
            ...
            <>
              <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm relative overflow-hidden">
                <div className="absolute top-3 right-3">
                  <button
                    onClick={() => { setUnlockedPatientIds(prev => prev.filter(id => id !== selectedPatientId)); setPasswordInput(''); setPasswordError(null); }}
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-mono font-bold px-1.5 py-0.5 bg-emerald-50 text-emerald-800 border border-emerald-150 rounded">
                        🔓 {getPatientFolderCode(selectedPatient)}
                      </span>
                    </div>
                    <p className="text-xs text-slate-500 mt-0.5">{selectedPatient.occupation} • Tlf: {selectedPatient.phone} • {selectedPatient.email}</p>
                  </div>
              ...
              <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-4">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-100">
                  <div>
                    <h2 className="text-xs font-bold uppercase tracking-wider text-slate-400">📁 Carpeta de Expediente</h2>
                ...
                <div className="space-y-2">
                  {filteredFiles.length === 0 ? (
                    <div className="py-10 text-center bg-slate-50/50 rounded-xl border border-dashed border-slate-200/80">
                      <p className="text-xs font-medium text-slate-400">Esta carpeta no posee archivos para este año.</p>
                    </div>
                  ) : (
                    filteredFiles.map((file) => {
                      ...
                      return (
                        <div key={file.id} id={`file_item_${file.id}`} className="border border-slate-100 rounded-xl bg-white hover:border-slate-200/80 transition-all overflow-hidden">
                          <div onClick={() => toggleFileContent(file.id)} className="p-3.5 flex items-center justify-between gap-4 cursor-pointer hover:bg-slate-50/50">
                            <div className="flex items-center gap-2.5">
                              <FileText className="h-4 w-4 text-slate-400" />
                              <div>
                                <div className="flex items-center gap-2 flex-wrap">
                                  <span className="text-xs font-bold text-slate-800">{file.name}</span>
                                  <span className="text-[9px] font-bold px-1.5 py-0.1 bg-indigo-50 text-indigo-700 rounded border border-indigo-100">{categoryLabel}</span>
                                </div>
              ...
              <div id="file_uploader_panel" className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => setShowUploadForm(!showUploadForm)} className="px-3 py-1.5 text-xs font-bold bg-emerald-50 text-emerald-800 border border-emerald-100 rounded-xl hover:bg-emerald-100 transition-all cursor-pointer">
                    {showUploadForm ? '✕ Cerrar formulario' : '➕ Nueva nota/documento'}
                  </button>
                </div>

                {statusMsg && (
                  <div className="p-3 rounded-xl text-xs font-bold bg-emerald-50 text-emerald-800 border-l-4 border-emerald-500">{statusMsg}</div>
                )}
                  ...
                  <form onSubmit={handleUploadSubmit} className="space-y-3 pt-2 border-t border-slate-100">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      ...
                      <div>
                        ...
                        <select id="file_category_select" value={uploadCategory} onChange={(e) => setUploadCategory(e.target.value)} className="w-full mt-1 px-3 py-1.5 text-xs border border-slate-200 rounded-lg text-slate-700 focus:outline-none focus:ring-1 focus:ring-emerald-500 bg-slate-50/50">
                    ...
                    <div className="flex flex-wrap items-center gap-1.5 mt-1.5 pt-1">
                      ...
                      <button type="button" onClick={() => selectQuickMockFile('Evolucion_Sesion_9.docx', 'El paciente asiste de forma puntual. Menciona reducción de pánico diario tras aplicar técnicas cognitivo-conductuales de anclaje.')} className="px-2 py-0.5 text-[9px] bg-white border border-slate-200 rounded text-slate-600 hover:bg-slate-50 cursor-pointer">
                        📄 Evolución Sesión
                      </button>
                      <button type="button" onClick={() => selectQuickMockFile('Resultado_Test_Ansiedad.pdf', 'Puntaje: Ansiedad leve. Se observa mejoría sobresaliente comparada con la sesión del mes anterior.')} className="px-2 py-0.5 text-[9px] bg-white border border-slate-200 rounded text-slate-600 hover:bg-slate-50 cursor-pointer">
                        📊 Test de Ansiedad
                      </button>
                    </div>
